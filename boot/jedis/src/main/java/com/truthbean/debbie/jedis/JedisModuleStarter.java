 ...
 * @since 0.6.3
 */
public class JedisModuleStarter implements DebbieModuleStarter {

    private static final String ENABLE_KEY = "debbie.jedis.enable";
    ...
    public void registerBean(ApplicationContext applicationContext, BeanInfoManager beanInfoManager) {
        DebbieReflectionBeanFactory<JedisConfiguration> debbieBeanInfo =
                new DebbieReflectionBeanFactory<>(JedisConfiguration.class);
        debbieBeanInfo.addBeanName("jedisConfiguration", JedisConfiguration.class.getName());
    ...
    public void starter(ApplicationContext applicationContext) {
        ...
        JedisConfiguration configuration = factory.factory(JedisConfiguration.class);
        JedisClientFactory clientFactory = new JedisClientFactory(configuration);
        var beanFactory = new SimpleBeanFactory<>(clientFactory, JedisClientFactory.class);
    ...
    public void release(ApplicationContext applicationContext) {
        var factory = applicationContext.getGlobalBeanFactory();
        JedisClientFactory clientFactory = factory.factory(JedisClientFactory.class);
