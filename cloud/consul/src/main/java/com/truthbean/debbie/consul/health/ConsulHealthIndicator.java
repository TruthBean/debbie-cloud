 ...
 */
package com.truthbean.debbie.consul.health;

import com.truthbean.debbie.consul.ConsulClient;

/**
 * Health indicator that checks whether the Consul agent is reachable.
 *
...
public class ConsulHealthIndicator {
    ...
    public boolean isHealthy() {
        return client.isAgentAvailable();
    }
    ...
    public HealthStatus check() {
        try {
            if (client.isAgentAvailable()) {
                ...
                return new HealthStatus(true, "consul agent reachable, leader=" + leader);
            }
            return new HealthStatus(false, "consul agent not reachable");
        } catch (Exception e) {
            return new HealthStatus(false, "consul health check failed: " + e.getMessage());
        }
    }
